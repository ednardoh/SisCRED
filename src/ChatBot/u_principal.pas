...
type
  TfrmPrincipal = class(TForm)
    ...
    TBDividas: TFDMemTable;
    ds_Dividas: TDataSource;
    TBDividascpf: TWideStringField;
    TBDividasnomecliente: TWideStringField;
    TBDividascpf_cnpj: TWideStringField;
    TBDividasrazaosocialcredor: TWideStringField;
    TBDividastipopgto: TWideStringField;
    TBDividasvalor_total: TWideStringField;
    TBDividasdt_primeira_parcela: TWideStringField;
    TBDividasqdiaspriparc: TIntegerField;
    TBDividasstatuspriparc: TWideMemoField;
    TBDividasvl_primeira_parcela: TWideStringField;
    TBDividasdt_segunda_parcela: TWideStringField;
    TBDividasqdiassegparc: TIntegerField;
    TBDividasstatussegparc: TWideMemoField;
    TBDividasvl_segunda_parcela: TWideStringField;
    TBDividasdt_terceira_parcela: TWideStringField;
    TBDividasqdiasterparc: TIntegerField;
    TBDividasstatusterparc: TWideMemoField;
    TBDividasvl_terceira_parcela: TWideStringField;
    TBDividasdt_quarta_parcela: TWideStringField;
    TBDividasqdiasquaparc: TIntegerField;
    TBDividasstatusquaparc: TWideMemoField;
    TBDividasvl_quarta_parcela: TWideStringField;
  ...
  public
    ...
    mensagem  : string;
    AFile     : string;
    AClienteTemDividas: Boolean;

    function IIf(pCond:Boolean;pTrue,pFalse:Variant): Variant;
    function ApenasNumerosStr(pStr:String): String;
    function ValidaCpf(CPF_Text: string): boolean;
    function IsNumber(strVal: string): string;
    function ValidaRespMenuBoasvindas(pMensagem : string) : Boolean;
    function RetornaSaudacao: string;
    function AutoAtendimentoVirtual( pMensagem, pSessao, pTelefone, pContato : String ) : Boolean;

    procedure PreencheDataSetDividas(strJSONValue: string);
    procedure BuscaDividas(strURL, strAText: string); //consultar dividas por CPF do Cliente
...
var
  I: Integer;
begin
  BoletoCredor := TFreeBoleto.create(application);
  ImgBoleto    := TFreeBoletoImagem.create(self);
  AClienteTemDividas := False;
...
procedure TfrmPrincipal.GerarBoleto;
var xBltAcordo:TFreeBoleto;
    x: Integer;
begin
  BoletoCredor.LimparTudo;
  xBltAcordo:=TFreeBoleto.create(nil);
  x := 0;
  TBDividas.First;
  while not TBDividas.Eof do
  begin
    // Beneficiario (Cedente)
    with BoletoCredor.Cedente do
    begin
      Agencia                  := '0001';
      CodigoBanco              := '001';
      Nome                     := TBDividasrazaosocialcredor.AsString;
      CodigoCedente            := '1000230';
      ContaCorrente            := '001000023';
      DigitoContaCorrente      := '9';
      Banco151.ModalidadeConta := '2';
      Banco001.Convenio        := '1';
      CodigoTransmissao        := '1';
      EnderecoCompleto         := 'Avenida Carlos Jereissati, 100 - Loja - 27, Jereissat Maracanau-CE';
      Pessoa                   := pJuridica;
      CNPJ_CPF                 := TBDividascpf_cnpj.AsString;
    end;


    // Titulo
    BoletoCredor.Especie         := 'R$';
    BoletoCredor.DataDocumento   := trunc(Now);
    BoletoCredor.Documento       := '0123456789';
    BoletoCredor.Vencimento      := now + 5;
    BoletoCredor.Valor           := StrToCurr(StringReplace(TBDividasvalor_total.AsString, '.', ',', [rfReplaceAll, rfIgnoreCase]));
    BoletoCredor.NossoNumero     := '4952';
    BoletoCredor.Instrucoes.Text := 'Pagável em qualquer banco até o vencimento.';
    BoletoCredor.Moeda           := '9';
    BoletoCredor.carteira        := '101';


    // Sacado
    with BoletoCredor.sacado do
    begin
      Nome     := TBDividasnomecliente.AsString;
      Endereco := 'Avenida Carlos Jereissati, 100';
      Bairro   := 'Centro';
      Cep      := '61901012';
      Estado   := 'CE';
      Cidade   := 'Maracanau';
      Pessoa   := pFisica;
      CNPJ_CPF := TBDividascpf.AsString;
    end;

    // CNAB
    BoletoCredor.CNAB.Ocorrencia := TOcorrenciasCNAB(GetEnumValue(TypeInfo(TOcorrenciasCNAB), 'oRegistrar'));
    BoletoCredor.CNAB.Baixar := TBaixarCNAB(GetEnumValue(TypeInfo(TBaixarCNAB), 'bBaixar'));
    BoletoCredor.CNAB.Protesto := TProtestoCNAB(GetEnumValue(TypeInfo(TProtestoCNAB), 'pNaoProtestar'));
    BoletoCredor.CNAB.TipoCobranca := TTipoCobrancaCNAB(GetEnumValue(TypeInfo(TTipoCobrancaCNAB), 'tcSimplesRapida'));
    BoletoCredor.CNAB.JurosDia := StrToCurr('0,00');
    BoletoCredor.CNAB.ValorDesconto := StrToCurr('0,00');
    BoletoCredor.CNAB.DataDesconto := now;
    BoletoCredor.CNAB.ValorIOF := StrToCurr('0,00');
    BoletoCredor.CNAB.ValorAbatimento := StrToCurr('0,00');
    BoletoCredor.CNAB.IdentTitulo := '';
    BoletoCredor.CNAB.ProtestarDias := StrToInt('0');
    BoletoCredor.CNAB.BaixarDias := StrToInt('5');
    BoletoCredor.CNAB.ValorMulta := StrToCurr('0,00');
    BoletoCredor.CNAB.DataMulta := now;

    BoletoCredor.preparar;

    CloneProperties(BoletoCredor,xBltAcordo);
    ImgBoleto.ListaBoletos.Add(xBltAcordo);

    ImgBoleto.SaveBoletoImagem(ExtractFilePath(Application.ExeName)+'Boletoacordo_'+IntToStr(x)+'.jpg',x);
...
function TfrmPrincipal.IIf(pCond: Boolean; pTrue, pFalse: Variant): Variant;
begin
  If pCond Then Result := pTrue
  else Result := pFalse;
end;

function TfrmPrincipal.IsNumber(strVal: string): string;
begin
...
procedure TfrmPrincipal.BuscaDividas(strURL, strAText: string);
var
  I: Integer;
  strJSON: string;
begin
  TBDividas.Close;
  TBDividas.Open;
  TBDividas.EmptyDataSet;
  TBDividas.IndexFieldNames := 'cpf';   //só para order por id
  TBDividas.Close;
  TBDividas.Open;
  ...
  if RESTResponse1.StatusCode in [200,201,204] then
    begin
      PreencheDataSetDividas(RESTResponse1.Content);  //processa o JSon
    end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if not AMessage.fromMe then  //Não exibe mensages enviadas por mim --por emquanto
          begin
            ...
            if chk_AutoResposta.Checked then
              AutoAtendimentoVirtual(AMessage.body, '', telefone, contato);
          end;
        end;
      end;
    end;
...
procedure TfrmPrincipal.PreencheDataSetDividas(strJSONValue: string);
var
...
begin
  ...
  for I := 0 to JsonObjArray.Count-1 do
    begin
      ...
      TBDividas.Insert;
      TBDividascpf.AsString                 := JsonObjectJson.GetValue('cpf').Value;
      TBDividasnomecliente.AsString         := JsonObjectJson.GetValue('nomecliente').Value;
      TBDividascpf_cnpj.AsString            := JsonObjectJson.GetValue('cpf_cnpj').Value;
      TBDividasrazaosocialcredor.AsString   := JsonObjectJson.GetValue('razaosocialcredor').Value;
      TBDividastipopgto.AsString            := JsonObjectJson.GetValue('tipopgto').Value;
      TBDividasvalor_total.AsString         := JsonObjectJson.GetValue('valor_total').Value;
      TBDividasdt_primeira_parcela.AsString := JsonObjectJson.GetValue('dt_primeira_parcela').Value;
      TBDividasqdiaspriparc.AsInteger       := JsonObjectJson.GetValue('qdiaspriparc').Value.ToInteger;
      TBDividasstatuspriparc.AsString       := JsonObjectJson.GetValue('statuspriparc').Value;
      TBDividasvl_primeira_parcela.AsString := JsonObjectJson.GetValue('vl_primeira_parcela').Value;
      TBDividasdt_segunda_parcela.AsString  := JsonObjectJson.GetValue('dt_segunda_parcela').Value;
      TBDividasqdiassegparc.AsInteger       := JsonObjectJson.GetValue('qdiassegparc').Value.ToInteger;
      TBDividasstatussegparc.AsString       := JsonObjectJson.GetValue('statussegparc').Value;
      TBDividasvl_segunda_parcela.AsString  := JsonObjectJson.GetValue('vl_segunda_parcela').Value;
      TBDividasdt_terceira_parcela.AsString := JsonObjectJson.GetValue('dt_terceira_parcela').Value;
      TBDividasqdiasterparc.AsInteger       := JsonObjectJson.GetValue('qdiasterparc').Value.ToInteger;
      TBDividasstatusterparc.AsString       := JsonObjectJson.GetValue('statusterparc').Value;
      TBDividasvl_terceira_parcela.AsString := JsonObjectJson.GetValue('vl_terceira_parcela').Value;
      TBDividasdt_quarta_parcela.AsString   := JsonObjectJson.GetValue('dt_quarta_parcela').Value;
      TBDividasqdiasquaparc.AsInteger       := JsonObjectJson.GetValue('qdiasquaparc').Value.ToInteger;
      TBDividasstatusquaparc.AsString       := JsonObjectJson.GetValue('statusquaparc').Value;
      TBDividasvl_quarta_parcela.AsString   := JsonObjectJson.GetValue('vl_quarta_parcela').Value;
      TBDividas.Post;
    end;

end;

function TfrmPrincipal.RetornaSaudacao: string;
begin
  if (FormatDateTime('hh:mm:ss',NOW)) > '00:00:01' then
  result:= 'Bom dia!';
  if (FormatDateTime('hh:mm:ss',NOW)) > '12:00:01' then
  result:= 'Boa Tarde!';
  if (FormatDateTime('hh:mm:ss',NOW)) > '18:30:01' then
  result:= 'Boa Noite!';
end;
...
function TfrmPrincipal.ValidaCpf(CPF_Text: string): boolean;
Var
  i,d,b,
  Digito : Byte;
  Soma : Integer;
...
begin
  Result := False;
  CPF_Text := ApenasNumerosStr(CPF_Text);
  // Caso o número não seja 11 (CPF) ou 14 (CNPJ), aborta
  Case Length(CPF_Text) of
    11: CNPJ := False;
    14: CNPJ := True;
  else Exit;
  end;
  ...
  DgCalc := '';
  DgPass := Copy(CPF_Text,Length(CPF_Text)-1,2);
  CPF_Text := Copy(CPF_Text,1,Length(CPF_Text)-2);
  // Calcula o digito 1 e 2
  For d := 1 to 2 do begin
    B := IIF(D=1,2,3); // BYTE
    SOMA := IIF(D=1,0,STRTOINTDEF(DGCALC,0)*2);
    for i := Length(CPF_Text) downto 1 do begin
      Soma := Soma + (Ord(CPF_Text[I])-Ord('0'))*b;
      Inc(b);
...
function TfrmPrincipal.ValidaRespMenuBoasvindas(pMensagem: string): Boolean;
var
  iMensagem : integer;
begin
  iMensagem := StrToInt(IsNumber(trim(pMensagem)));
  case iMensagem of
    1:result := True;
    2:result := True;
    3:result := True;
...
function TfrmPrincipal.AutoAtendimentoVirtual(pMensagem, pSessao, pTelefone,
...
begin
  ...
  x := 0;
   if ( POS('OLA', AnsiUpperCase(pMensagem))        > 0 ) or ( POS('OLÁ', AnsiUpperCase(pMensagem))       > 0 ) or
      ...
      ( POS('INICIO', AnsiUpperCase(pMensagem))     > 0 ) or ( POS('OI', AnsiUpperCase(pMensagem))        > 0 ) or
      ( POS('DIVIDAS', AnsiUpperCase(pMensagem))    > 0 ) or ( POS('INCIAR', AnsiUpperCase(pMensagem))    > 0 )then
      begin
        mensagem :=
        TInject1.Emoticons.AtendenteH+ RetornaSaudacao +' *'+pContato+'!*\n\n'+
        'Eu sou seu atendente virtual, e me chamo Mark, estou aqui para te ajudar!\n\n'+
        'Escolha uma opção:\n\n'+
        TInject1.Emoticons.Um             +' Consultar dividas\n\n'+
        TInject1.Emoticons.Dois           +' Atendimento ATH\n\n'+
        TInject1.Emoticons.Tres           +' Endereço\n\n'+
        TInject1.Emoticons.Quatro         +' Horários\n\n';
        TInject1.SendFile(pTelefone, ExtractFileDir(Application.ExeName)+'\Img\logobot.jpg', mensagem);

      end
   else
   begin
      if (trim(pMensagem) = '1') or (trim(pMensagem) = '2') or
         (trim(pMensagem) = '3') or (trim(pMensagem) = '4') and
         (not AClienteTemDividas) then
      begin
        if not ValidaRespMenuBoasvindas(pMensagem) then
        begin
          TInject1.Send(pTelefone,
                        TInject1.Emoticons.Naoutilizado+
                        TInject1.Emoticons.Naoutilizado+
                        TInject1.Emoticons.Naoutilizado+
          ...
          begin
            if trim(pMensagem) = '1' then  //consulta dividas por CPF
            begin
              TInject1.Send(pTelefone, TInject1.Emoticons.LoiraNotebook+' Muito bom! Agora você pode me informar o seu CPF?');
            end;
          end;
      end;


      if (Length(trim(pMensagem)) > 1) and (Length(trim(pMensagem)) = 11) then  //valida cpf
      begin
        if ValidaCpf(trim(pMensagem)) then
        begin
          BuscaDividas('http://localhost:9000/consultardividas/', trim(pMensagem)); //consulta dividas na api
          if TBDividas.RecordCount = 0 then  //Não achou dividas
          begin
            TInject1.Send(pTelefone, TInject1.Emoticons.Apaixonado+
                                     TInject1.Emoticons.Apaixonado+
                                     TInject1.Emoticons.Apaixonado+
                                     ' Parabens! Não foram encontrado dividas em seu CPF!');
          end
          else
          begin
            sMensagem :='';
            ...
            sMensagem := '```';
            sMensagem := sMensagem + '======================'+'\n';
            sMensagem := sMensagem + 'CPF : '+ TBDividascpf.AsString+'  '+'Cliente: '+TBDividasnomecliente.AsString+'\n';
            sMensagem := sMensagem + '======================'+'\n';
            sMensagem := sMensagem + 'DIVIDAS COM CREDORES : '+'\n';
            sMensagem := sMensagem + '======================'+'\n';
            while not TBDividas.Eof do
            begin
              sMensagem := sMensagem + 'CNPJ: *'+ TBDividascpf_cnpj.AsString+'* Credor: *'+TBDividasrazaosocialcredor.AsString+'*\n';
              sMensagem := sMensagem + 'Topo pgto: '+TBDividastipopgto.AsString+' Valor Total : '+TBDividasvalor_total.AsString+'\n';
              sMensagem := sMensagem + '======================'+'\n';
              sMensagem := sMensagem + ' Dt.Primeira parc. : '+ TBDividasdt_primeira_parcela.AsString+'\n'+
                                       ' Qtde.Dias Atraso : '+ TBDividasqdiaspriparc.AsString+'\n'+
                                       ' Valor Primeira parc. : '+ TBDividasvl_primeira_parcela.AsString+'\n';
              sMensagem := sMensagem + ' Dt.Segunda parc. : '+ TBDividasdt_segunda_parcela.AsString+'\n'+
                                       ' Qtde.Dias Atraso : '+ TBDividasqdiassegparc.AsString+'\n'+
                                       ' Valor Segunda parc. : '+ TBDividasvl_segunda_parcela.AsString+'\n';
              if TBDividasdt_terceira_parcela.AsString <> '' then
              begin
                sMensagem := sMensagem + ' Dt.Terceira parc. : '+ TBDividasdt_terceira_parcela.AsString+'\n'+
                                         ' Qtde.Dias Atraso  : '+ TBDividasqdiasterparc.AsString+'\n'+
                                         ' Valor Segunda parc. : '+ TBDividasvl_terceira_parcela.AsString+'\n';
              end;

              if TBDividasdt_quarta_parcela.AsString <> '' then
              begin
                sMensagem := sMensagem + ' Dt.Quarta parc. : '+TBDividasdt_quarta_parcela.AsString+'\n'+
                                         ' Qtde.Dias Atraso : '+ TBDividasqdiasquaparc.AsString+'\n'+
                                         ' Valor Quarta parc. : '+ TBDividasvl_quarta_parcela.AsString+'\n';
              end;
            ...
            sMensagem := sMensagem + ' '+'\n\n';
            sMensagem := sMensagem +
            TInject1.Emoticons.Sorridente+' *'+pContato+'!*\n'+
            'Você deseja fazer um acordo?\n\n'+
            TInject1.Emoticons.Um             +' Sim\n\n'+
            TInject1.Emoticons.Dois           +' Não\n\n';
            AClienteTemDividas := True;
            sMensagem := sMensagem + '```';
            TInject1.Send(pTelefone,sMensagem);
          end;
        end
        else
        begin
          TInject1.Send(pTelefone, TInject1.Emoticons.Triste+
                                   TInject1.Emoticons.Triste+
                                   TInject1.Emoticons.Triste+
                                   ' Seu CPF é invalido!');
          Result := True;
          exit;
        ...
        if (trim(pMensagem) = '2') and (not AClienteTemDividas) then
        begin
          TInject1.Send(pTelefone, TInject1.Emoticons.Like+
                                   TInject1.Emoticons.Like+
                                   TInject1.Emoticons.Like+
                                   ' Ok então! Vou te encaminhar para o Atendimento Humano!\n\n'+
                                   TInject1.Emoticons.LoiraAteLogo+
                                   '  Então você já sabe! Se precisar estou por aqui... é só me chamar!'+
                                   TInject1.Emoticons.LoiraOla+' Tchau!!!');
          Result := True;
          exit;
        end
      else
        if (trim(pMensagem) = '3') and (not AClienteTemDividas) then
        begin
          TInject1.Send(pTelefone, TInject1.Emoticons.Sorridente+
                                   ' Nosso Endereço é: Rua Johen Carneiro, 256 - Lidice ' +
                                   ' - CEP: 38.400-070 - Uberlândia/MG');
          Result := True;
          exit;
        end
      else
        if (trim(pMensagem) = '4') and (not AClienteTemDividas) then
        begin
          TInject1.Send(pTelefone, TInject1.Emoticons.Sorridente+
                                   ' Nosso horário de funcionamento e: '+
                                   ' das 09h às 11h e das 15h às 17h de seg a sex ');
          Result := True;
          exit;
        end;

      if AClienteTemDividas then
      begin
        if (POS('SIM', AnsiUpperCase(pMensagem))>0) or
           (POS('S', AnsiUpperCase(pMensagem))>0) then
        begin
          AClienteTemDividas := False;
          GerarBoleto;
          TBDividas.First;
          while not TBDividas.Eof do
          begin
            sNomeBoleto := ExtractFilePath(Application.ExeName)+'Boletoacordo_'+IntToStr(x)+'.jpg';
            TInject1.SendFile(pTelefone, sNomeBoleto, '');
            TBDividas.Next;
            x := x + 1;
          end;
          TInject1.Send(pTelefone, TInject1.Emoticons.Apaixonado+
                                   TInject1.Emoticons.Apaixonado+
                                   TInject1.Emoticons.Apaixonado+
                                   ' Parabens!!! Seu boleto será gerado agora.');
          Result := True;
          exit;
        end
        else
          if (POS('NAO', AnsiUpperCase(pMensagem))>0) or
             (POS('NÃO', AnsiUpperCase(pMensagem))>0) or
             (POS('N', AnsiUpperCase(pMensagem))>0) then
        begin
          TInject1.Send(pTelefone, TInject1.Emoticons.Like+
                                   TInject1.Emoticons.Like+
                                   TInject1.Emoticons.Like+
                                   'Ok então! Você já sabe! Se precisar estou por aqui... é só me chamar!'+
                                   TInject1.Emoticons.LoiraOla+' Tchau!!!');
          AClienteTemDividas := False;
          Result := True;
          Exit;
...
procedure TfrmPrincipal.CloneProperties(SourceComp, DestComp: TObject);
var
  Propinfo: PPropInfo;
