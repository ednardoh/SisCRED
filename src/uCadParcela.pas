  ...
  if strStatusCli ='dsInserindo' then
    begin
      try
        ...
        oJson.AddPair('valor_total'        , StringReplace(Edt_Valortot.Value.ToString, ',', '.', [rfReplaceAll, rfIgnoreCase]));
        oJson.AddPair('dt_primeira_parcela', RetornaDataNull(MKE_primeira_Parc.Text));
        oJson.AddPair('vl_primeira_parcela', StringReplace(Edt_Valor_primeira_parc.Value.ToString, ',', '.', [rfReplaceAll, rfIgnoreCase]));
        oJson.AddPair('dt_segunda_parcela' , RetornaDataNull(MKE_segunda_Parc.Text));
        oJson.AddPair('vl_segunda_parcela' , StringReplace(Edt_Valor_segunda_parc.Value.ToString, ',', '.', [rfReplaceAll, rfIgnoreCase]));
        oJson.AddPair('dt_terceira_parcela', RetornaDataNull(MKE_terceira_Parc.Text));
        oJson.AddPair('vl_terceira_parcela', StringReplace(Edt_Valor_terceira_parc.Value.ToString, ',', '.', [rfReplaceAll, rfIgnoreCase]));
        oJson.AddPair('dt_quarta_parcela'  , RetornaDataNull(MKE_quarta_Parc.Text));
        oJson.AddPair('vl_quarta_parcela'  , StringReplace(Edt_Valor_quarta_parc.Value.ToString, ',', '.', [rfReplaceAll, rfIgnoreCase]));
...
begin
  ...
  BuscaTipopagto('http://localhost:9000/tipopgtobyid/', trim(Edt_Tipopgto.Text));
...
begin
  ...
  for I := 0 to JsonObjArray.Count-1 do
    begin
      ...
      TBParcelasvalor_total.AsString           := StringReplace(JsonObjectJson.GetValue('valor_total').Value, '.', ',', [rfReplaceAll, rfIgnoreCase]);;
      TBParcelasdt_primeira_parcela.AsString   := JsonObjectJson.GetValue('dt_primeira_parcela').Value;
      TBParcelasvl_primeira_parcela.AsString   := StringReplace(JsonObjectJson.GetValue('vl_primeira_parcela').Value, '.', ',', [rfReplaceAll, rfIgnoreCase]);
      TBParcelasdt_segunda_parcela.AsString    := JsonObjectJson.GetValue('dt_segunda_parcela').Value;
      TBParcelasvl_segunda_parcela.AsString    := StringReplace(JsonObjectJson.GetValue('vl_segunda_parcela').Value, '.', ',', [rfReplaceAll, rfIgnoreCase]);
      TBParcelasdt_terceira_parcela.AsString   := JsonObjectJson.GetValue('dt_terceira_parcela').Value;
      TBParcelasvl_terceira_parcela.AsString   := StringReplace(JsonObjectJson.GetValue('vl_terceira_parcela').Value, '.', ',', [rfReplaceAll, rfIgnoreCase]);
      TBParcelasdt_quarta_parcela.AsString     := JsonObjectJson.GetValue('dt_quarta_parcela').Value;
      TBParcelasvl_quarta_parcela.AsString     := StringReplace(JsonObjectJson.GetValue('vl_quarta_parcela').Value, '.', ',', [rfReplaceAll, rfIgnoreCase]);
